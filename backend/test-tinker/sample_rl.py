...
def main() -> None:
    ...
    ap.add_argument("--log-path", required=True, help="Path to RL training log dir")
    ap.add_argument("--model-name", help="Base model name (overrides config.json)")
    ap.add_argument("--prompt", help="Prompt to sample (try asking for strict JSON)")
    args = ap.parse_args()
    ...
    sampling_params = tinker.SamplingParams(
        max_tokens=int(os.getenv("RL_MAX_TOKENS", "256")),
        temperature=float(os.getenv("RL_TEMPERATURE", "0.4")),
