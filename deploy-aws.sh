...
load_from_dotenv \
  ...
  TERMINAL_IMAGE TERMINAL_CPU TERMINAL_MEMORY TERMINAL_PIDS_LIMIT TERMINAL_IDLE_TIMEOUT_SECONDS TERMINAL_BLOCK_PRIVATE_NETS \
...
# Terminal (App 9)
TERMINAL_IMAGE=${TERMINAL_IMAGE:-ubuntu:22.04}
TERMINAL_CPU=${TERMINAL_CPU:-1}
TERMINAL_MEMORY=${TERMINAL_MEMORY:-1g}
TERMINAL_PIDS_LIMIT=${TERMINAL_PIDS_LIMIT:-256}
TERMINAL_IDLE_TIMEOUT_SECONDS=${TERMINAL_IDLE_TIMEOUT_SECONDS:-900}
TERMINAL_BLOCK_PRIVATE_NETS=${TERMINAL_BLOCK_PRIVATE_NETS:-true}
...
kubectl create secret generic app-secrets \
    ...
    --from-literal=TERMINAL_IMAGE="${TERMINAL_IMAGE:-ubuntu:22.04}" \
    --from-literal=TERMINAL_CPU="${TERMINAL_CPU:-1}" \
    --from-literal=TERMINAL_MEMORY="${TERMINAL_MEMORY:-1g}" \
    --from-literal=TERMINAL_PIDS_LIMIT="${TERMINAL_PIDS_LIMIT:-256}" \
    --from-literal=TERMINAL_IDLE_TIMEOUT_SECONDS="${TERMINAL_IDLE_TIMEOUT_SECONDS:-900}" \
    --from-literal=TERMINAL_BLOCK_PRIVATE_NETS="${TERMINAL_BLOCK_PRIVATE_NETS:-true}" \
    | kubectl apply -f -
